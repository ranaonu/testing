		...
		<div class="relative flex flex-col px-10 py-8 lg:flex-row">
			<div class="flex justify-start w-full mb-8 lg:w-3/12 xl:w-1/5 lg:m-b0">
				<div class="relative w-32 h-32 cursor-pointer group">
					<img id="preview" src="{{url(auth()->user()->avatar)}}" class="w-32 h-32 rounded-full ">
				...
				<div class="row mt-5">
						...
						<div class="form-group">
							...
							<select class="form-control form-select disable_delivery_info msDropdown" required="" name="shipper_country" id="shipper_country" onChange="getCountry(this.value, 'shipper')">
								...
								@foreach ($countries as $country)
									<option value="{{$country->alpha_2_code}}" data-image="{{ asset('themes/' . $theme->folder . '/images/flags/'.strtolower($country->alpha_2_code).'.png') }}" {{(isset(Auth::user()->shipper_country) && Auth::user()->shipper_country == $country->alpha_2_code)?'selected':''}}>{{$country->country_name}}</option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="col-lg-8">
						<div class="form-group">
							...
							<input type="text" class="form-control disable_delivery_info" required="" name="shipper_address" id="shipper_address" placeholder="Please enter your address" value="{{ Auth::user()->shipper_address }}">   
						</div>
					</div>
				</div>    
				<div class="row">
					<div class="col-lg-4">
						<div class="form-group">
							...
							<input type="text" class="form-control disable_delivery_info" required="" name="shipper_zip" id="shipper_zip" value="{{ Auth::user()->shipper_zip }}">   
						</div>
					</div>
					<div class="col-lg-4">
						<div class="form-group">
							<label class="form-label">City <span class="required">*</span></label>                        
							<input type="text" class="form-control disable_delivery_info" required="" name="shipper_city" id="shipper_city" value="{{ Auth::user()->shipper_city }}">
						</div>
					</div>
					<div class="col-lg-4">
						<div class="form-group">
							<label class="form-label">State <span class="required">*</span></label>                       
							<input type="text" class="form-control disable_delivery_info" required="" name="shipper_state" id="shipper_state" value="{{ Auth::user()->shipper_state }}">  
						</div>
					</div>
				</div>

				<div class="mt-1">
					<label for="shiper_phone" class="block text-sm font-medium leading-5 text-gray-700">Phone Number</label>
					<div class="mt-1 rounded-md shadow-sm">
						<input id="shipper_phone" type="tel" name="shipper_phone" placeholder="phone" value="{{ Auth::user()->shipper_phone }}" required class="w-full form-input">
					</div>
				</div>

				<div class="mt-5">
					<label for="about" class="block text-sm font-medium leading-5 text-gray-700">About</label>
					<div class="mt-1 rounded-md text-area-height">
