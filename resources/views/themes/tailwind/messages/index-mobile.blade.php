...
<main class="page-content">
  <div class="page-title">
    <div class="d-flex align-items-center">
      <span>@lang('labels.backend.messages.title')</span>
            ...
            <div class="messaging">
                <div class="inbox_msg">
                            ...
                            @if($threads->count() > 0)
                                @foreach($threads as $item)
                                    @if($item->latestMessage)
                                        <a  href="{{route('student.messages').'?thread='.$item->id}}" class="@if($item->userUnreadMessagesCount(auth()->user()->id)) unread
                                            @endif">
                    ...
                    @if(request()->has('thread'))
                        <form method="post" class="compose-reply" action="{{route('student.messages.reply')}}">
                            @csrf
                                ...
                                <div class="chat_people box-header">
                                    ...
                                    <div class="chat_ib float-left">
                                        ...
                                        <p class="float-right d-inline mb-0">
                                            <a class="" href="{{route('student.messages',['thread'=>$thread->id])}}">
                    ...
                    @endif
                    <form method="post" class="d-none compose-message" action="{{route('student.messages.send')}}">
                        @csrf
